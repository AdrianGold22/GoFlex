...
class Crud_model extends CI_Model {
	...
	*/
	...
	function get_links($movie_id = '')
	{   $list_field='url';
		$links = $this->db->get_where('movie', array('movie_id'=>$movie_id))->row()->$list_field;
		$links_array= json_decode($links);
		$keys = array_keys($links_array); 
		
		return $keys;

	}
	function get_links_url($movie_id = '')
	{
		$list_field='url';
		$links = $this->db->get_where('movie', array('movie_id'=>$movie_id))->row()->$list_field;
		$links_array= json_decode($links);
        $value = array_values($links_array);
		return $value;
	}
